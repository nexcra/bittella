...
import jBittorrentAPI.PeerProtocol;
import java.io.*;
        ...
        while (this.run) {
            ...
            try {
                if (!hsOK) {
                    ...
                    if ((read = this.dis.read(lengthHS)) > 0) {
                        for (int i = 0; i < 19; i++)
                            protocol[i] = (byte) dis.read();
                        for (int i = 0; i < 8; i++)
                            reserved[i] = (byte) dis.read();
                        for (int i = 0; i < 20; i++)
                            fileID[i] = (byte) dis.read();
                        for (int i = 0; i < 20; i++)
                            peerID[i] = (byte) dis.read();
                    ...
                    int id;
                    if ((read = this.dis.read(length)) > 0) {
                        l = Utils.byteArrayToInt(length);
                        System.out.println("Message Read Length= "+(new java.math.BigInteger(length)).toString(16));
                        if (l == 0) {
                            mess.setData(PeerProtocol.KEEP_ALIVE);
                        } else {
                            id = dis.read();
                            System.out.println("Message Read Type= "+id);
                            if(id == -1){
                                ...
                                if (l == 1)
                                    ...
                                    if (this.read(payload) > 0){
                                        mess.setData(id, payload);
                                        System.out.println(payload.toString());
                                        System.out.println(mess.getPayload().toString());
                                        for(int j = 0;j<payload.length;j++){
                                            System.out.print((new java.math.BigInteger(new byte[]{payload[j]})).toString(16));
                                        }
                                        System.out.println("");
                                    }else{payload = null;}
                                }
            ...
            if(!this.hsOK){
                ...
                this.hsOK = true;
                System.out.println("HS Read at Transport Level");
            }else{
