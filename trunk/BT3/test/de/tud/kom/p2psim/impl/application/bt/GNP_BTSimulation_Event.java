...
public class GNP_BTSimulation_Event extends SimulatorTest implements SupportOperations {
	...
	public void setUp() {
		...
		this.netFactory.setGnpFile("config/gnp/7391_0.2.xml");
	...
	public void makeFullTorrent(long startTimeWindow, String documentName, long fileSize, long theDuration, long seederDuration, long leecherDuration) {
		...
		for (int i = 0; i < numberOfLeecher; i++)
			leecherStartTimes.addLast(startTimeOffset + (i * (startTimeWindow / numberOfLeecher))); //This results in a (nearly) equidistant number of start times.
		...
		BTOperationPeerStarter<GNP_BTSimulation_Event> peerStarter = new BTOperationPeerStarter<GNP_BTSimulation_Event>(leecherStartTimes, leecher, torrent, this, this);
	...
	public static void main(String[] args) {
		int days = 7;
		String filename = "outputs/" + args[1];
		...
		GNP_BTSimulation_Event.runSimulation(filename, fileSize, duration, seederDuration, leecherDuration, startWindow);
	}
	...
	public static void runSimulation(String filename, long fileSize, long duration, long seederDuration, long leecherDuration, long startWindow) {
		...
		//Before
		GNP_BTSimulation_Event testClass;
		testClass = new GNP_BTSimulation_Event();
...
//					try { resultFile.close(); if (textFile != null) textFile.close(); } catch (IOException e2) {/*Nothing to do*/}
				try { if (textFile != null) textFile.close(); } catch (IOException e2) {/*Nothing to do*/}
			}
		}
	}
