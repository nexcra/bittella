...
public class GnpNetBandwidthManagerEvent extends AbstractGnpNetBandwidthManager {
	...
	private static Logger log = SimLogger.getLogger(GnpNetBandwidthManagerEvent.class);
	...
	public GnpNetBandwidthManagerEvent() {
		tempSenderConnections = new HashMap<NetLayer, Set<GnpNetBandwidthAllocation>>(connectionsSenderToReceiver.size());
		tempReceiverConnections = new HashMap<NetLayer, Set<GnpNetBandwidthAllocation>>(connectionsReceiverToSender.size());
	...
	public void allocateBandwidth() {
		...
		for (NetLayer nl : connectionsSenderToReceiver.keySet()) {
			tempSenderConnections.put(nl,new HashSet<GnpNetBandwidthAllocation>(connectionsSenderToReceiver.get(nl).size()));
			tempSenderConnections.get(nl).addAll((Collection<GnpNetBandwidthAllocation>) connectionsSenderToReceiver.get(nl).values());
			tempConnections.addAll(connectionsSenderToReceiver.get(nl).values());
		}
		for (NetLayer nl : connectionsReceiverToSender.keySet()) {
			tempReceiverConnections.put(nl, new HashSet<GnpNetBandwidthAllocation>(connectionsReceiverToSender.get(nl).size()));
			tempReceiverConnections.get(nl).addAll(connectionsReceiverToSender.get(nl).values());
		}
		...
		while (!tempConnections.isEmpty()) {
			...
			for (NetLayer s : senders) {
				...
				generateMeanBids(s.getCurrentUploadBandwidth(), temp, true);
			}
			for (NetLayer r : receivers) {
				Set<GnpNetBandwidthAllocation> temp = new HashSet<GnpNetBandwidthAllocation>();
				temp.addAll(tempReceiverConnections.get(r));
				generateMeanBids(r.getCurrentDownloadBandwidth(), temp, false);
			}
...
*/
	 ...
	 * 
	 * Example: sender s has an bandwith of 30bytes/second and 3 receivers r1-r3.
	 * Normally each connection will be assigned with 10 bytes/s
	 * 
	 * If the throughput of connection s -> r1 is limited to 6 bytes/s =>
	 * s -> r1 : 6 bytes/s
	 * s -> r1 : 12 bytes/s
	 * s -> r1 : 12 bytes/s
	 * 
	 * @param bandwidth available for bids
	 * @param unassigned Allocations will be empty after run
	 * @param isSender true if allocations relates to one sender, false if allocations relates to one reciever
	 * @return number of smaller opposite bids
	 */
	private int generateMeanBids(double bandwidth, Collection<GnpNetBandwidthAllocation> unassigned, boolean isSender) {
		if (unassigned.isEmpty())
			return 0;

		// find connections with an upper bound of throughput that is below fair fraction of bandwidth
	...
	private boolean generateMinimumBids(NetLayer p, boolean sender) {
		...
		Set<GnpNetBandwidthAllocation> connections = (sender) ? tempSenderConnections.get(p) : tempReceiverConnections.get(p);
		...
		double bandwidth = (sender) ? p.getCurrentUploadBandwidth() : p.getCurrentDownloadBandwidth();
		Set<GnpNetBandwidthAllocation> temp = new HashSet<GnpNetBandwidthAllocation>();
...
//			double bandwidth = (sender) ? p.getCurrentUploadBandwidth() : p.getCurrentDownloadBandwidth();
			double min = slowestConnections.iterator().next().getPreviousBid(!sender, step);
...
//		System.out.println(bidSender + ":" + c.isBidRepeated(true) + " - " + bidReceiver + ":" + c.isBidRepeated(false));
		...
		if (bidSender <= bidReceiver && c.isMinBid(true) && c.isBidRepeated(true)) {
			...
			((AbstractNetLayer) c.getSender()).setCurrentUpBandwidth(c
					.getSender().getCurrentUploadBandwidth()
					- bidSender);
			((AbstractNetLayer) c.getReceiver()).setCurrentDownBandwidth(c
					.getReceiver().getCurrentDownloadBandwidth()
					- bidSender);
			return true;
